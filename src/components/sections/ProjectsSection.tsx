...
const PROJECTS: MissionProject[] = [
  {
    ...
    description: 'A credential card system built on Go microservices, using OpenAI GPT-4o to enrich card content.',
    fullDescription:
      'A Proof-of-Expertise card system I worked on at KGeN. It exposes 6 REST APIs in Go, with DynamoDB and AWS S3 for storage, and OpenAI GPT-4o generating enriched card content. I focused on the API design, the GPT-4o prompt templates, and the DynamoDB access patterns — and learned a lot about shipping an AI feature reliably.',
    type: 'Product Feature',
    status: 'DEPLOYED',
    tech: ['Go', 'DynamoDB', 'AWS S3', 'OpenAI GPT-4o', 'REST API'],
    impact: '6 REST APIs with GPT-4o enrichment',
    impactMetrics: [
  ...
  },
  {
    ...
    description: 'Redis distributed locking that removes race conditions in social-login (OAuth) token flows.',
    fullDescription:
      'I implemented a Redis-backed distributed lock on top of the NestJS + AWS Cognito OAuth flow. Concurrent token exchanges were occasionally creating duplicate user records; atomic SETNX locks with a TTL keep it to a single writer per user across Google, Apple, and Discord login. This was a great lesson in how subtle concurrency bugs show up only under load.',
    type: 'Security Infrastructure',
    status: 'DEPLOYED',
    tech: ['Redis', 'NestJS', 'AWS Cognito', 'DynamoDB', 'OAuth'],
    impact: 'Removed duplicate-record race conditions in OAuth',
    impactMetrics: [
      { label: 'Race Conditions', value: '0' },
      { label: 'Login Providers', value: '3' },
  ...
  },
  {
    ...
    description: 'A Go + PostgreSQL service that supports parallel annotation tracks across 5+ Indian languages.',
    fullDescription:
      'A Go service I worked on at KGeN that helps coordinate annotation work across multiple Indian languages. It distributes tasks to language-specific tracks and keeps progress in PostgreSQL so several languages can run in parallel. Building it taught me a lot about structuring a service and modeling its state cleanly.',
    type: 'Platform Engineering',
    status: 'DEPLOYED',
    tech: ['Go', 'PostgreSQL', 'REST APIs', 'Microservices'],
    impact: '5+ Indian languages in parallel annotation tracks',
    impactMetrics: [
      { label: 'Languages', value: '5+' },
      { label: 'Parallel Tracks', value: '5' },
      { label: 'Data Store', value: 'PG' },
    ],
    architectureNodes: [
      { id: 'client', label: 'Client', sublabel: 'API', color: '#A78BFF' },
      { id: 'orch', label: 'Service', sublabel: 'Go', color: '#A78BFF' },
      { id: 'workers', label: 'Lang Tracks', sublabel: 'x5', color: '#34D399' },
      { id: 'pg', label: 'PostgreSQL', sublabel: 'State', color: '#61DAFB' },
    ],
    architectureEdges: [
      { from: 'client', to: 'orch', label: 'API' },
      { from: 'orch', to: 'workers', label: 'Dispatch' },
      { from: 'orch', to: 'pg', label: 'State' },
      { from: 'workers', to: 'pg', label: 'Progress' },
    ],
    challenges: [
      'Designing a task schema flexible enough to cover different annotation task types',
      'Keeping progress state consistent while several language tracks run in parallel',
      'Learning to structure a Go service so it stays readable as it grows',
    ],
  },
  {
