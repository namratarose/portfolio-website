...
const NODES: NodeDef[] = [
  ...
  },
  {
    ...
    sublabel: 'Reward events',
    icon: '⬟',
    x: 380,
    y: 200,
    color: '#F77F00',
    glow: 'rgba(247,127,0,0.4)',
    description: 'Processes in-game reward events and payouts, using idempotency keys and retries so nothing is double-counted.',
    tech: 'Go + DynamoDB',
    layer: 'service',
  },
  {
...
const PATTERNS: PatternCardProps[] = [
  ...
  },
  {
    ...
    badge: 'Async',
    badgeColor: '#7C5CFF',
    description: 'Services communicate via SQS/SNS message queues for decoupled, reliable async processing. Reward and notification events trigger Lambda workers without blocking the request path.',
    points: [
  ...
  {
    index: 2,
    title: 'Resilient Retries & Idempotency',
    badge: 'Resilience',
    badgeColor: '#F77F00',
    description: 'Important operations retry safely using idempotency keys, so a retried request never double-applies. Failed work falls back to alternate paths instead of erroring out.',
    points: [
